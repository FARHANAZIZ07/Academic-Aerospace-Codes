# -*- coding: utf-8 -*-
"""
Created on Wed Nov  15 21:11:36 2023
...
L = 1  # m # Defining length between surfaces
N_rays = 1000000  # Defining number of rays emitted, needs to be large
...
# Setting list to store impacted surfaces
...
for i in range(N_rays):
    ...
    xhat_ray = np.sin(theta) * xhat90 + np.cos(theta) * yhat90  # From Eq (6)
    yhat_ray = np.sin(theta) * yhat90 + np.cos(theta) * xhat90  # From Eq (6)
...
# Calculate the view factor F from Eq (10)
view_factor = num_impacted_rays / total_rays

# Printing the view factor into Console
print(f'View Factor (F) for {N_rays} rays =', view_factor)
